...
const UserAppoinment = ({ appointmentList, loading }) => {
  ...
  console.log(allAppint?.[0].doctorData);
  return (
    <section className="sec-com" id="chamber-list">
          ...
          <table>
            <thead>
              <tr className="table100-head">
                ...
                <th className="column2">Doctor Name</th>
                ...
                <tbody>
                  {allAppint?.map((data, ind) => (
                    <tr key={data._id}>
                      ...
                      <td className="column2"> {data?.doctorData?.name}</td>
                      <td className="column3"> {data?.doctorData?.phoneNum}</td>
      ...
      {popup && (
        <PopupModal
          ...
          clsBtn={setPopup}
          message={`You want to cancel Appointment?`}
        ></PopupModal>
      )}
    </section>
  );
